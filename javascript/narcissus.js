    ...
    if(detections != undefined ){
        if(detections.multiFaceLandmarks != undefined && detections.multiFaceLandmarks.length >= 1){
            ...
            if(detectionData != undefined){
                let emotions = detectionData.expressions;
                if(emotions != undefined){

                    let treshold = 0.75;

                    let angry = detectionData.expressions.angry;
                    let disgusted = detectionData.expressions.disgusted;
                    let fearful = detectionData.expressions.fearful;
                    let happy = detectionData.expressions.happy;
                    let neutral = detectionData.expressions.neutral;
                    let sad = detectionData.expressions.sad;
                    let surprised = detectionData.expressions.surprised;

                    let lastEmotionDatabase = neutralDatabase;
                    let lastSourceImageXPosition = width * 4;

                    if(angry > treshold){
                        if(seed < 0.6){
                            displayFace(fearfulDatabase, width * 2);
                            lastEmotionDatabase = fearfulDatabase;
                            lastSourceImageXPosition = width * 2;
                        } else{
                            displayFace(angryDatabase, 0);
                            lastEmotionDatabase = angryDatabase;
                            lastSourceImageXPosition = 0;
                        }
                    ...
                    else if(disgusted > treshold) {
                        if(seed < 0.6){
                            displayFace(sadDatabase, width * 5);
                            lastEmotionDatabase = sadDatabase;
                            lastSourceImageXPosition = width * 5;
                        } else{
                            displayFace(fearfulDatabase, width * 2);
                            lastEmotionDatabase = fearfulDatabase;
                            lastSourceImageXPosition = width * 2;
                    ...
                    else if(fearful > treshold){
                        if(seed < 0.6){
                            displayFace(angryDatabase, 0);
                            lastEmotionDatabase = angryDatabase;
                            lastSourceImageXPosition = 0;
                        } else{
                            displayFace(surprisedDatabase, width * 6);
                            lastEmotionDatabase = surprisedDatabase;
                            lastSourceImageXPosition = width * 6;
                        }
                    ...
                    else if(happy > treshold){
                        displayFace(happyDatabase, width * 3);
                        lastEmotionDatabase = happyDatabase;
                        lastSourceImageXPosition = width * 3;

                        //console.log('happy');
                    }


                    else if(neutral > treshold){
                        if(seed < 0.6){
                            displayFace(sadDatabase, width * 5);
                            lastEmotionDatabase = sadDatabase;
                            lastSourceImageXPosition = width * 5;
                        } else{
                            displayFace(neutralDatabase, width * 4);
                            lastEmotionDatabase = neutralDatabase;
                            lastSourceImageXPosition = width * 4;
                        }
                        ...
                        displayFace(lastEmotionDatabase, lastSourceImageXPosition);
                    }
                }
            }
        }
...
//neutralFace => (width * 4)
//sadFace => (width * 5)
//surprisedFace => (width * 6)

function displayFace(emotionDatabase, sourceImageXPosition){
    ...
    for(a = 0; a < faceCountour.length; a++){

        let vertexX = detections.multiFaceLandmarks[0][faceCountour[a]].x;
        let vertexY = detections.multiFaceLandmarks[0][faceCountour[a]].y;
        let vertexZ = detections.multiFaceLandmarks[0][faceCountour[a]].z;
    
        maskCanvas.vertex(vertexX * width, vertexY * height, vertexZ);
    }
    ...
    for(a = 0; a < faceCountour.length; a++){
        let sourceImageVertexX = emotionDatabase[faceCountour[a]].x;
        let sourceImageVertexY = emotionDatabase[faceCountour[a]].y;

        let vertexX = detections.multiFaceLandmarks[0][faceCountour[a]].x;
        let vertexY = detections.multiFaceLandmarks[0][faceCountour[a]].y;
        let vertexZ = detections.multiFaceLandmarks[0][faceCountour[a]].z;
    
        faceCanvas.vertex(vertexX * width, vertexY * height, vertexZ, sourceImageVertexX * width + sourceImageXPosition, sourceImageVertexY * height);
    }
    ...
    blurV.setUniform('texelSize', [2.0 / width, 2.0 / height]);
...
displayAngryFaceV1 = function(){
    
    beginShape(TRIANGLES);
    texture(personPicture);
    
    for(let h = 0; h < tassellationDatabase.length; h++){
        let sourceImageVertex1 = angryDatabase[tassellationDatabase[h][0]];
        let sourceImageVertex2 = angryDatabase[tassellationDatabase[h][1]];
        let sourceImageVertex3 = angryDatabase[tassellationDatabase[h][2]];
        
        let vertex1X = detections.multiFaceLandmarks[0][tassellationDatabase[h][0]].x;
        let vertex1Y = detections.multiFaceLandmarks[0][tassellationDatabase[h][0]].y;
        let vertex2X = detections.multiFaceLandmarks[0][tassellationDatabase[h][1]].x;
        let vertex2Y = detections.multiFaceLandmarks[0][tassellationDatabase[h][1]].y;
        let vertex3X = detections.multiFaceLandmarks[0][tassellationDatabase[h][2]].x;
        let vertex3Y = detections.multiFaceLandmarks[0][tassellationDatabase[h][2]].y;

        vertex(vertex1X * width, vertex1Y * height, sourceImageVertex1.x * width, sourceImageVertex1.y * height);
        vertex(vertex2X * width, vertex2Y * height, sourceImageVertex2.x * width, sourceImageVertex2.y * height);
        vertex(vertex3X * width, vertex3Y * height, sourceImageVertex3.x * width, sourceImageVertex3.y * height);
    }

    endShape();
}

displayDisgustedFaceV1 = function(){
    
    beginShape(TRIANGLES);
    texture(personPicture);
    
    for(let h = 0; h < tassellationDatabase.length; h++){
        let sourceImageVertex1 = disgustedDatabase[tassellationDatabase[h][0]];
        let sourceImageVertex2 = disgustedDatabase[tassellationDatabase[h][1]];
        let sourceImageVertex3 = disgustedDatabase[tassellationDatabase[h][2]];
        ...
        vertex(vertex1X * width, vertex1Y * height, sourceImageVertex1.x * width + width, sourceImageVertex1.y * height);
        vertex(vertex2X * width, vertex2Y * height, sourceImageVertex2.x * width + width, sourceImageVertex2.y * height);
        vertex(vertex3X * width, vertex3Y * height, sourceImageVertex3.x * width + width, sourceImageVertex3.y * height);
    }

    endShape();
}

displayFearfulFaceV1 = function(){
    
    beginShape(TRIANGLES);
    texture(personPicture);
    
    for(let h = 0; h < tassellationDatabase.length; h++){
        let sourceImageVertex1 = fearfulDatabase[tassellationDatabase[h][0]];
        let sourceImageVertex2 = fearfulDatabase[tassellationDatabase[h][1]];
        let sourceImageVertex3 = fearfulDatabase[tassellationDatabase[h][2]];
        ...
        vertex(vertex1X * width, vertex1Y * height, sourceImageVertex1.x * width + (width * 2), sourceImageVertex1.y * height);
        vertex(vertex2X * width, vertex2Y * height, sourceImageVertex2.x * width + (width * 2), sourceImageVertex2.y * height);
        vertex(vertex3X * width, vertex3Y * height, sourceImageVertex3.x * width + (width * 2), sourceImageVertex3.y * height);
    }

    endShape();
}

displayHappyFaceV1 = function(){
    
    beginShape(TRIANGLES);
    texture(personPicture);
    
    for(let h = 0; h < tassellationDatabase.length; h++){
        let sourceImageVertex1 = happyDatabase[tassellationDatabase[h][0]];
        let sourceImageVertex2 = happyDatabase[tassellationDatabase[h][1]];
        let sourceImageVertex3 = happyDatabase[tassellationDatabase[h][2]];
        ...
        vertex(vertex1X * width, vertex1Y * height, sourceImageVertex1.x * width  + (width * 3), sourceImageVertex1.y * height);
        vertex(vertex2X * width, vertex2Y * height, sourceImageVertex2.x * width + (width * 3), sourceImageVertex2.y * height);
        vertex(vertex3X * width, vertex3Y * height, sourceImageVertex3.x * width + (width * 3), sourceImageVertex3.y * height);
    }

    endShape();
}

displayNeutralFaceV1 = function(){
    
    beginShape(TRIANGLES);
    texture(personPicture);
    
    for(let h = 0; h < tassellationDatabase.length; h++){
        let sourceImageVertex1 = neutralDatabase[tassellationDatabase[h][0]];
        let sourceImageVertex2 = neutralDatabase[tassellationDatabase[h][1]];
        let sourceImageVertex3 = neutralDatabase[tassellationDatabase[h][2]];
        ...
        vertex(vertex1X * width, vertex1Y * height, sourceImageVertex1.x * width  + (width * 4), sourceImageVertex1.y * height);
        vertex(vertex2X * width, vertex2Y * height, sourceImageVertex2.x * width  + (width * 4), sourceImageVertex2.y * height);
        vertex(vertex3X * width, vertex3Y * height, sourceImageVertex3.x * width  + (width * 4), sourceImageVertex3.y * height);
    }

    endShape();
}

displaySadFaceV1 = function(){
    
    beginShape(TRIANGLES);
    texture(personPicture);
    
    for(let h = 0; h < tassellationDatabase.length; h++){
        let sourceImageVertex1 = sadDatabase[tassellationDatabase[h][0]];
        let sourceImageVertex2 = sadDatabase[tassellationDatabase[h][1]];
        let sourceImageVertex3 = sadDatabase[tassellationDatabase[h][2]];
        ...
        vertex(vertex1X * width, vertex1Y * height, sourceImageVertex1.x * width  + (width * 5), sourceImageVertex1.y * height);
        vertex(vertex2X * width, vertex2Y * height, sourceImageVertex2.x * width  + (width * 5), sourceImageVertex2.y * height);
        vertex(vertex3X * width, vertex3Y * height, sourceImageVertex3.x * width  + (width * 5), sourceImageVertex3.y * height);
    }

    endShape();
}

displaySurprisedFaceV1 = function(){
    
    beginShape(TRIANGLES);
    texture(personPicture);
    
    for(let h = 0; h < tassellationDatabase.length; h++){
        let sourceImageVertex1 = surprisedDatabase[tassellationDatabase[h][0]];
        let sourceImageVertex2 = surprisedDatabase[tassellationDatabase[h][1]];
        let sourceImageVertex3 = surprisedDatabase[tassellationDatabase[h][2]];
        ...
        vertex(vertex1X * width, vertex1Y * height, sourceImageVertex1.x * width  + (width * 6), sourceImageVertex1.y * height);
        vertex(vertex2X * width, vertex2Y * height, sourceImageVertex2.x * width  + (width * 6), sourceImageVertex2.y * height);
        vertex(vertex3X * width, vertex3Y * height, sourceImageVertex3.x * width  + (width * 6), sourceImageVertex3.y * height);
    }
